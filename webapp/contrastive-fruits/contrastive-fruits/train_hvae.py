...
class StylePairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        c_path = random.choice(self.content_paths)
...
def parse_args():
    ...
    p.add_argument('--pair-mode', type=str, default='random', choices=['random', 'all'],
...
def train():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pbar = tqdm(loader, desc=f"HVAE Epoch {epoch}/{args.epochs}")
        running = 0.0
        batch_count = 0
        for batch in pbar:
            if args.pair_mode == 'all':
